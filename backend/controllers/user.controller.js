...
export const updateUserProfile = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ success: true, message: "Profile update successfully", user });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
};
...
export const getResume = async (req, res) => {
  try {
    ...
    if (!publicId) {
      return res
        .status(400)
        .json({ success: false, message: "Invalid Resume reference" });
    }
    ...
    if (resourceType === "raw") {
      ...
      const signedUrl = cloudinary.utils.private_download_url(
        ...
        format,
        {
          ...
          type: "upload",
          secure: true,
          expires_at: Math.floor(Date.now() / 1000) + 300, // URL expires in 5 minutes
        },
      );
    ...
    return res
      .status(500)
      .json({ success: false, message: "Could not access the resume" });
  }
};
